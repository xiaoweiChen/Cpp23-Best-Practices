...
一种固定大小、基于栈的连续容器。数据大小必须在编译时已知，并且必须有足够的栈空间来容纳数据。这种容器有助于我们优先使用栈而非堆。由于其位置已知且内存连续，std::array<> 成为了一种“零开销抽象”。编译器可以执行一系列优化，因为它知道数据的大小和位置。
...
几乎任何容器的使用都需要添加注释并说明原因。对于小型容器，使用带线性搜索的扁平化映射(flat map)很可能比 std::map 更好。
...
\begin{itemize}
\item
 Bjarne Stroustrup “列表是邪恶的吗？”\footnote{\url{https://www.stroustrup.com/bs_faq.html\#list}}
\end{itemize}
