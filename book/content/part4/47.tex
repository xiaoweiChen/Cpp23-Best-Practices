...
\begin{myTip}{}
使用 auto 可以避免不必要的转换和数据丢失。这与范围 for 循环的道理相同。auto 要求必须进行初始化，这一点和 const 一样，其背后的原因也相同。
\end{myTip}
...
get\_string\_value() 的返回类型是什么？如果它是 std::string\_view 或 const char *，那么在所有编译器上都会产生一次潜在的、代价高昂的转换，且不会有诊断提示。
...
这段代码强制我们对分子和分母使用相同的类型(可以试试看，会得到一些奇怪的编译错误)。
...
但在 C++14 中，可以完全省略返回类型(记住，跳过 C++11)。
...
在 C++20 中，可以使用 auto 函数参数进一步简化这段代码(这会创建隐式模板)
...
上面的练习非常适合整理成一系列实验，并保存在 GitHub 的代码片段(gists)中，正如《C++ 并非魔法》一书中所提到的那样。
\end{myTip}
...
\item
“(几乎)总是使用 auto”(AAA，Almost Always Auto)\footnote{\url{https://herbsutter.com/2013/08/12/gotw-94-solution-aaa-style-almost-always-auto/}}
\end{itemize}
